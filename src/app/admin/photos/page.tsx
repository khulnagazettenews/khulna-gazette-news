  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(err.error || 'Failed to delete photo.');
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xs border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-red-600 mb-1">
            <Camera size={16} />
            <span>Photo Features Management</span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Upload and feature high quality photo gallery images and captions on homepage.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold text-slate-700">
            <div>
              <label className="block mb-1.5 font-extrabold text-slate-900">Gallery Image File <span className="text-red-600">*</span></label>
              {imageUrl ? (
                ...
                <label className="aspect-video border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-red-600 transition bg-slate-50">
                  {uploading ? (
                    <span className="text-xs font-bold text-red-600 animate-pulse">Uploading...</span>
                  ) : (
                    <>
                      <Upload size={22} className="text-slate-400 mb-1.5" />
                      <span className="text-xs font-bold text-slate-600">Select Photo</span>
                    </>
                  )}
            ...
            <div>
              <label className="block mb-1.5 font-bold text-slate-900">Photo Caption (Optional)</label>
              <input
                type="text"
                value={caption}
                onChange={(e) => setCaption(e.target.value)}
                placeholder="e.g. Scenic sunset view of Rupsha River"
            ...
            <div>
              <label className="block mb-1.5 font-bold text-slate-900">Photo Credit (Optional)</label>
              <input
                type="text"
                value={credit}
                onChange={(e) => setCredit(e.target.value)}
                placeholder="e.g. Khulna Gazette / Staff Photographer"
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:outline-none focus:border-red-500 focus:bg-white transition"
              />
            </div>
        ...
        <div className="lg:col-span-7 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
          <h3 className="font-extrabold text-slate-900 text-base border-b border-slate-100 pb-3 flex items-center justify-between">
            <span>Photo Gallery Items</span>
            <span className="text-xs text-slate-400 font-bold">Total: {photos.length} Photos</span>
          </h3>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-400 font-medium">No photos found.</div>
          ) : (
                  ...
                  <div>
                    ...
                    {photo.caption && (
                      <div className="p-3">
                        ...
                        {photo.credit && <span className="text-[10px] text-slate-400 font-semibold block mt-1">Photo: {photo.credit}</span>}
                      </div>
                    )}
                  </div>
